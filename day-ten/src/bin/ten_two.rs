...
fn main() {
    ...
    let solution = grid
        ...
        .enumerate()
        .map(|(row_idx, row)| {
            // start on the outside and count after crossing to inside areas
            let mut inside = false;
            row.iter()
                .enumerate()
                .filter(|&(col_idx, c)| {
                    if grid_loop.contains(&(row_idx, col_idx)) {
                        if matches!(c, '|' | 'J' | 'L') {
                            inside = !inside; // crossed over
                        }
                        false // on a pipe cell, don't count
                    } else {
                        inside // count if inside
                    }
                })
                .count()
        })
