fn main() {
    ...
    let solution = (0..galaxies.len())
        .map(|i| {
            ((i + 1)..galaxies.len())
                .map(|oi| {
                    let gal_y = galaxies[i][0] + row_zeroes[galaxies[i][0]];
                    let gal_x = galaxies[i][1] + column_zeroes[galaxies[i][1]];

                    let other_y = galaxies[oi][0] + row_zeroes[galaxies[oi][0]];
                    let other_x = galaxies[oi][1] + column_zeroes[galaxies[oi][1]];
